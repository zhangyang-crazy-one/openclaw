...
def check_data_source_status():
    ...
    try:
        ...
        try:
            ...
            em_status = "degraded"
            em_msg = f"eastmoney blocked: {str(e)[:50]}"
        
        # 测试新浪 (sina) - HTTP 直连
        try:
            headers = {'Referer': 'http://finance.sina.com.cn', 'User-Agent': 'Mozilla/5.0'}
            resp = req.get('https://hq.sinajs.cn/list=sh600000', headers=headers, timeout=5)
            if resp.status_code == 200 and '"' in resp.text:
                sina_status = "ok"
                sina_msg = "sina-http ok"
            else:
                sina_status = "error"
                sina_msg = "sina-http returned invalid data"
        except Exception as e:
            sina_status = "error"
            sina_msg = f"sina-http failed: {str(e)[:50]}"
        
        if em_status == "ok":
            results["akshare"] = {"status": "ok", "message": "连接成功"}
        elif sina_status == "ok":
            results["akshare"] = {"status": "degraded", "message": f"eastmoney blocked, sina-http 可用"}
        else:
            results["akshare"] = {"status": "error", "message": f"全部通道失败: eastmoney[{em_msg}], sina[{sina_msg}]"}
    except Exception as e:
