...
"""
A股资产负债表补全脚本
使用 Sina API 获取资产负债表数据

用法:
  python fetch_balance_sheet.py [并发数] [批次大小]
  python fetch_balance_sheet.py 8      # 8并发，无限批次
  python fetch_balance_sheet.py 8 1000 # 8并发，每次最多1000只

进度文件: balance_progress.json（自动记录已完成的股票）
"""
...
BALANCE_FILE = FINANCIAL_DIR / "balance.csv"
PROGRESS_FILE = FINANCIAL_DIR / "balance_progress.json"
...
def fetch_balance_sheet(symbol: str) -> pd.DataFrame | None:
    try:
...
def fetch_with_retry(symbol: str, max_retries: int = 2) -> pd.DataFrame | None:
    for attempt in range(max_retries):
        result = fetch_balance_sheet(symbol)
...
def update_balance_sheet(concurrency: int = 8, batch_size: int = None):
    print("=" * 60)
    print("📋 A股资产负债表补全")
    ...
    if all_results:
        ...
        if BALANCE_FILE.exists():
            try:
                existing = pd.read_csv(BALANCE_FILE)
        ...
        combined.to_csv(BALANCE_FILE, index=False, encoding='utf-8-sig')
        print(f"✅ 已保存: {BALANCE_FILE}")
...
if __name__ == "__main__":
    ...
    update_balance_sheet(concurrency=concurrency, batch_size=batch_size)
