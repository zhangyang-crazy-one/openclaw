...
## IL-020: 修图任务交付教训 (2026-07-02 16:18, 用户主动终止)

**场景**:

- 8h+ 修图任务 (08:00-16:18), 两个 workflow 交织: 暖调磨皮 (A) + 黑长直换发型 (B)
- 多次迭代生成 v1-v27, 最后阶段做换脸 (face-oval-only)
- 临近结束时用户问 "V22 是什么样子" → 我发原图 2832x3776 (829KB) → 用户看不到
- 我又压 1500 宽 (458KB) 重发 → 用户"修图任务结束" 终止

**根因**:

1. **🔴 图片交付标准缺失**: 没有"修图任务必须压缩预览" 的明确规则, 直接发 2.8M 像素大图, QQ 端显示/缓冲有问题
2. **🔴 版本号对用户不透明**: 8h+ 跑出 27+ 版本, 用户分不清 v22 (磨皮暖调) vs v23 (发型AI) vs v25-v27 (换脸) 的关系
3. **🔴 任务节点未做"3选1"checkpoint**: v23 / v25 / v27 是 3 个关键节点, 都没主动问用户 "这版要不要定下来"
4. **🟠 任务时长失控**: 8h+ 修图占用主会话, P0 债 (HEARTBEAT 蒸馏 / MEMORY 蒸馏) 全跳票

**Lesson (本条)**:

1. **🟠 修图/AI 生成任务交付清单**:
   - ✅ **必须压缩预览** (≤1500 宽, ≤500KB) 再发, 绝不发原图
   - ✅ **附带版本定位说明** ("v22 = 你原图磨皮暖调" vs "v23 = AI 换发型")
   - ✅ **关键节点 3 选 1**: 风格明显分叉时停下问用户, 不要一跑到底
2. **🟠 任务时长预警**:
   - 单一任务 >3h 应主动汇报进度 + 给出退出选项
   - 任务 >5h 应强制 P0 债窗口 (主会话修图跑太久, P0 跳票累积)
3. **🟠 用户体验优先级**:
   - **用户能"看到效果" > AI 模型效果**: 看不到 = 没做
   - 修图前要测 QQ 端能否正常显示 (不是测本地能开)
4. **🟢 换脸技术 v3 face-oval-only 成功**:
   - cv2 + alpha blending + 椭圆 mask + 色彩迁移 (LAB) → 保住原图脸
   - 限制: 脸型大小有偏差 (v27 用户脸 > v23 脸), 需进一步脸型对齐
5. **🟢 决策树更新**:
   - 用户要"换发型保脸" → 推荐 美图秀秀 AI 换发型 (30s, 专模型)
   - 用户要"换发型可失脸" → 我用 minimax 跑风格预览
   - 用户要"局部换 (只换头发区)" → cv2 face-oval swap 可行 (本次成功)

**Action**:

- [P1 必兑现 7/3 之前] 把"修图任务压缩预览 + 3选1checkpoint + 任务时长预警" 写入 self-improving/workflow.md (如果有)
- [P2 长期] 修图任务 SOP: v0=读图 / v1=压1500预览先发 / 后续迭代版本化
- [本次已记录到 memory/2026-07-02.md 16:18 entry]

## 2026-07-02 22:17

### IL-020: push2 "RECOVERED" 误判 — 单次 N/N 不充分, 24h 抽样才稳

**Context**:

- 06:18 凌晨心跳用 throttled polling 测得 push2.eastmoney.com 5/5 HTTP 200, 推"RECOVERED"结论并写入 HEARTBEAT
- 22:17 (16h 后) 再次抽样 0/3 endpoint (push2 + push2his), 全部 HTTP 000 / TLS eof
- 切换 http1.1/http2/UA 均无效
- 与 19 日 rapid burst 错方法不同, 这是新 signature

**Lesson**:

- "throttled 5/5 成功" 在单次短窗口测试中无法证明 endpoint 健康
- eastmoney 的 anti-scraping 可能有时段策略 (白天 anti / 凌晨放行?) 或 IP reputation 漂移
- **修订 IL-017**: push2 间歇性可用, 不是 RECOVERED; 仓位信心中坚回 qt 单源 (累计稳态 7.8+ 日)
- **规则**: 任何 "endpoint X RECOVERED" 声明必须配 24h 内 ≥3 时段抽样 (晨/午/晚) ≥80% 成功率才可强声明; 单次窗口 N/N 仅可记 "transient OK"

**Action**:

- 仓位决策、量化报告数据源声明明确标注 "qt 单源" 而非 "qt + push2 双源"
- 下次任何 endpoint 状态评估默认 24h 抽样 3 次

### IL-021: 300276 MACD 死叉深检延误第 9 日 — 量化"回避"标的自动深检规则

**Context**:

- 7/2 16:13 量化报告 quant_analysis_2026-07-02.md 已明确评级三丰智能 (300276) "回避" 总分 ≤1
- 7/2 22:17 心跳仍未做 MACD 死叉深检
- 7/1 收 7.32 (+2.09%) 反弹迷惑 → 7/2 收 7.13 (-2.60%) 回归跌势
- 持仓盲飞第 9 日

**Lesson**:

- 量化报告 "回避" 信号已具备决策分量, 但无自动触发深检机制
- 持仓决策与量化报告 6h+ 间隔 = 风险敞口
- **规则**: 量化报告标"回避"的标的, 若当日 22:17 心跳前未做 MACD 深检, 应在次日开盘前自动减仓 1/3 防御, 不等深检结论

**Action**:

- 7/3 09:30 开盘前必做 300276 MACD 深检 + 持仓决策
- 后续 cron: 量化报告生成后 30min 内触发 "回避" 标的 MACD 深检 (paper_search_hybrid 模式复用)

### IL-022: cron 6h 周期 skip 累积 — daemon 稳态 ≠ jobs 健康

**Context**:

- 06:18 → 22:17 间隔 16h, 跨 6/2 12h/18h 两个 6h 周期
- cron daemon pid 1605 26d+ uptime 稳态
- 但 jobs 实际触发情况未验证

**Lesson**:

- daemon 进程健康 ≠ 调度系统健康
- jobs.json 累积漂移可导致 trigger 漏跑但 daemon 仍在线
- **规则**: 任何 cron 心跳间隔 > 12h 应在 entry 中标注"cron 漂移待查", 并跑 `openclaw cron list` 比对 enabled vs last_run

**Action**:

- 下次 heartbeat 间隔 > 12h 时, 跑 `openclaw cron list` 验证 jobs 状态
