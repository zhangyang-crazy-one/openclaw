...
# 匹配 node 启动的 openclaw gateway 进程（systemd 启动时命令行为 node .../openclaw/dist/index.js gateway）
GATEWAY_PID=$(pgrep -f "openclaw/dist/index\.js gateway" | head -1)
GATEWAY_PORT=18789
...
if [ -z "$GATEWAY_PID" ]; then
    ...
    pkill -9 -f "openclaw/dist/index\.js gateway" 2>/dev/null || true
    sleep 1
