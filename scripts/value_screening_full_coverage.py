...
def analyze_stock_financial(group: pd.DataFrame, cfg: ScoringConfig) -> Dict:
    ...
    # 每股收益和每股净资产
    epsjb = latest['EPSJB'] if not pd.isna(latest['EPSJB']) else 0
    bps = latest['BPS'] if not pd.isna(latest['BPS']) else 0
...
def screen_all_stocks(financial_df: pd.DataFrame, cfg: ScoringConfig) -> pd.DataFrame:
    ...
    for i, code in enumerate(stock_codes):
        ...
        try:
            ...
            # ✅ 全覆盖模式：不过滤任何股票，给所有股票打分
            # 即使基本面较差也会得分（0分），确保全部覆盖
...
def main():
    ...
    print("=" * 70)
    print("📊 A股量化打分系统 - 全覆盖版本 (V3报告框架)")
    print("   ✅ 不过滤任何股票 - 所有股票都会被评分")
    print("=" * 70)
    ...
    output_file = os.path.join(OUTPUT_DIR, f'value_screening_full_top200_{timestamp}.csv')
    top200 = result_df.head(200)
    top200.to_csv(output_file, index=False, encoding='utf-8-sig')
    print(f"\n💾 Top 200 已保存: {output_file}")
    
    output_all = os.path.join(OUTPUT_DIR, f'value_screening_full_all_{timestamp}.csv')
    result_df.to_csv(output_all, index=False, encoding='utf-8-sig')
    print(f"💾 全量 {len(result_df)} 只股票已保存: {output_all}")
