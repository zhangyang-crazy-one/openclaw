...
def fetch_all_data(batch_size=50):
    """获取全量数据"""
    # 读取股票列表
    ...
    for i, code in enumerate(codes):
        ...
        # 每批次保存
        if (i + 1) % batch_size == 0:
            save_data(all_financial, all_dividends)
            all_financial = []
            all_dividends = []
            print(f"已处理 {i+1} 只股票")
...
def save_data(financial_list, dividend_list):
    """保存数据"""
    if financial_list:
        df = pd.concat(financial_list, ignore_index=True)
        df.to_csv(DATA_DIR / "chuangye_financial_indicators.csv", index=False, encoding='utf-8-sig')
        print(f"已保存 {len(df)} 条财务指标数据")
    
    if dividend_list:
        df = pd.concat(dividend_list, ignore_index=True)
        df.to_csv(DATA_DIR / "chuangye_dividend.csv", index=False, encoding='utf-8-sig')
